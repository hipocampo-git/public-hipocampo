...
export class ImportClass {
  ...
  renameConflictingDeck = async (deckName) => {
    ...
    if (args.renameConflict || args.deleteConflict) {
      // Check if there's an existing deck
      const response = await axios.get(
        `${apiUrl}/decks?deckName=${deckName}&ownerId=${this.deckOwner.id}&simple=true`,
        axiosOptions
      );
      ...
      if (response.status === 200) {
        ...
        if (response.data.length > 1) {
          ...
          throw new Error('Multiple decks with the same name found');
        ...
        if (args.renameConflict) {
          ...
          // Delete the conflicting deck
          logger.info('Deleting the conflicting deck and cards');

          // Delete the cards first
          const response3 = await axios.get(
            `${apiUrl}/cards?deckId=${response.data[0].id}`,
            axiosOptions
          );
          ...
          if (response3.status === 200) {
            for (let i = 0; i < response3.data.data.length; i++) {
              const response4 = await axios.delete(
                `${apiUrl}/cards/${response3.data.data[i].id}`,
                axiosOptions
              );
          ...
          if (response2.status === 200) {
            logger.info('Conflicting deck deleted');
          }
        }
      }
    }
  ...
  createDeck = async (deck) => {
    ...
    try {
      const payload = {
        ...
        synopsis: deck.synopsis || undefined,
  ...
  createShare = async (share) => {
    ...
    if (share) {
      ...
      const payload = {
        checkedAdmin: _.isBoolean(share.defaultIsAdminMode)
          ? share.defaultIsAdminMode : true,
        checkedRandom: _.isBoolean(share.defaultIsRandomMode)
          ? share.defaultIsRandomMode : false,
        checkedSaveResponses: _.isBoolean(share.defaultIsSaveResponsesMode)
          ? share.defaultIsSaveResponsesMode : false,
        checkedTextToSpeech: _.isBoolean(share.defaultIsTextToSpeechMode) ?
          share.defaultIsTextToSpeechMode : false,
