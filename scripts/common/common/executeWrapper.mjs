...
export const runWrapper = async (callback) => {
  ...
  parser.add_argument('-dc', '--deleteConflict', {
    action: 'store_true',
    default: false,
    help: 'Delete the existing deck if there is a conflict.'
  });
  ...
  parser.add_argument('-ld', '--logDir', {
    default: null,
    help: 'Optional custom subdirectory for logs.'
  });
  ...
  console.log('logdir: ', args.logDir);

  exit();


  const logger = initLogs(topPath, args.logDir);
  ...
  const ownerUsername = args.owner || args.user;

  // if (args.owner) {
    const response2 = await axios.get(
      `${apiUrl}/users?search=${ownerUsername}`,
      axiosOptions
    );
